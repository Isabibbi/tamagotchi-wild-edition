...
Real-time observability of the multi-agent system is provided by a dedicated web dashboard strictly decoupled from the core simulation runtime. The visualization architecture is organized into four complementary components:

\subsection{The Web Presentation Framework (NiceGUI)}
For the presentation tier, \textbf{NiceGUI} was adopted---a Python web framework built on top of FastAPI and Vue.js. This choice was driven by the objective of maintaining the entire codebase within the Python ecosystem, while ensuring:
\begin{itemize}
    \item \textbf{Real-Time Synchronization}: state updates are pushed through bidirectional, persistent \textbf{WebSocket} connections, eliminating HTTP polling overhead;
    \item \textbf{Native Vector Rendering}: the interface directly manipulates \textbf{SVG} elements, enabling dynamic rendering of the grid floor plan without external JavaScript build toolchains.
\end{itemize}

\subsection{Inter-Process Communication and Decoupling (\texttt{gui\_bridge.py})}
The asynchronous event loop of SPADE (bound to XMPP network I/O) features concurrency constraints incompatible with the NiceGUI web server runtime. Running both subsystems within the same event loop would induce severe thread contention, where frontend rendering latency or browser disconnections could stall agent asynchronous tasks.
To guarantee strict \textbf{execution domain isolation}, \texttt{gui\_bridge.py} establishes an \textbf{Inter-Process Communication (IPC)} bridge:
\begin{itemize}
    \item Executes the multi-agent simulation in a dedicated, isolated process;
    \item Encapsulates world state snapshots emitted by the \texttt{VisualizationAgent} and streams them across an authenticated local socket connection (\texttt{multiprocessing.connection});
    \item Enforces fault tolerance: client rendering delays or browser reconnections cannot stall state transitions within the multi-agent environment.
\end{itemize}

\subsection{Graphical Presentation Layer (\texttt{visualization/nicegui\_view.py})}
This module implements the user-facing web frontend, translating state payloads delivered by the bridge into interactive graphical components:
\begin{itemize}
    \item \textbf{Interactive SVG Floor Plan}: geometrically renders the $12 \times 8$ grid, color-coding the four operational areas and static objects (bowls, medical examination stations);
    \item \textbf{Dynamic Entity Tracking}: tracks operator coordinates in real time and projects animal clinical status and species via dedicated icons;
    \item \textbf{Event and Diagnostic Timeline}: chronologically logs physical actions accepted or rejected by the environment;
    \item \textbf{Metrics and Control Panel}: exposes playback speed selection, simulation visualization pause/resume controls, and aggregated counters for patient health and inventory stocks.
\end{itemize}

\subsection{Role of the \texttt{VisualizationAgent} within the Pipeline}
Within the SPADE multi-agent network, the \texttt{VisualizationAgent} (introduced in Section~\ref{subsec:visualization_agent}) operates strictly as a passive sensor: devoid of a BDI deliberative engine or \texttt{.asl} plans, it listens for periodic broadcast messages emitted by the \texttt{EnvironmentAgent} under the \texttt{cras.visualization} ontology, validates schema compliance (\texttt{VisualizationUpdate}), and delegates frame forwarding to the IPC bridge.

\paragraph{Methodological Note on Tooling}
The architectural design of the multi-process asynchronous bridge and the dynamic vector SVG layout were developed with the assistance of Large Language Models (LLMs) used as a pair-programming and rapid-prototyping tool for the interface scaffolding.
