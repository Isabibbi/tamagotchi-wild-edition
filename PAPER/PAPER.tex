...
through a multi-agent lens. The facility is divided into four operational areas: 
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \texttt{HealthStatus}: finite health states (\texttt{HEALTHY}, \texttt{HUNGRY}, \texttt{SICK}, \texttt{IN\_TREATMENT}, \texttt{TREATED}, \texttt{IN\_OUTBOUND\_TRANSPORT}, \texttt{IN\_RETURN\_TRANSPORT}).
    \end{itemize}
...
\begin{itemize}
    \item \textbf{The \texttt{VeterinaryAgent} and its BDI plan (\texttt{veterinary.asl})}: the center's doctor was introduced, which receives the notification of a sick animal, requests transport from logistics, retrieves the medicine from the medical storage, and administers the treatment in the treatment room.
    ...
    \item \textbf{The animal finite state machine}: the animal does not heal instantaneously, but traverses a strict sequence of 6 states verified by the \texttt{EnvironmentAgent}:
    \begin{itemize}
...
Before describing the agents and their tasks, it is necessary to define the structure of the world and the physical rules that govern the rescue center. This part is written in pure Python and is completely independent of the network (SPADE/XMPP), BDI logic, and graphics.
...
\begin{itemize}
    ...
    \item \textbf{Animal Life Cycle (\texttt{HealthStatus})}: The sick animal does not heal instantly, but goes through a mandatory sequence of six states controlled by the environment:
\begin{center}
\small
...
\begin{itemize}
    \item \textbf{Action Types (\texttt{ActionType})}: List of all permitted operations, such as reserving a task (\texttt{CLAIM\_TASK}), entering or leaving a room (\texttt{ACQUIRE\_AREA}, \texttt{RELEASE\_AREA}), taking resources (\texttt{TAKE\_FOOD}, \texttt{TAKE\_MEDICINE}), filling bowls (\texttt{FILL\_BOWL}), and treating the animal (\texttt{TREAT\_ANIMAL}).
...
\end{itemize}
\subsection{The Authoritative Environment (\texttt{environment/state.py})}
...
\end{itemize}
The \texttt{EnvironmentAgent} does not make use of the \textbf{\texttt{agree}} performative. Indeed, an \texttt{agree} message expresses the formal acceptance of a task that will require time to complete (a deferred commitment over time, typical of operators such as the \texttt{FeedingAgent}). Conversely, the Environment's action is atomic and instantaneous: validation and state transition occur immediately, making an intermediate execution promise meaningless.
...
What it does: It is the declarative logical mind of the agent. It performs no calculations or network transfers: it limits itself to monitoring beliefs and adopting goals and intentions.
\begin{lstlisting}
!boot.
+!boot <-
    .mark_bdi_ready.
...
\end{itemize}
\paragraph{The Python Executive File: \texttt{src/tamagotchi\_wild/agents/feeding\_agent.py}}
...
\end{itemize}
\paragraph{The Python Executive File: \texttt{src/tamagotchi\_wild/agents/logistics\_agent.py}}
...
\subsubsection{Whom the Agent Communicates With}
...
\end{enumerate}
\paragraph{B. The Python Executive File: \texttt{src/tamagotchi\_wild/agents/veterinary\_agent.py}}
...
\begin{itemize}
    ...
    \item \textbf{Transport Request} (\textbf{\texttt{RequestTransport}}): Requests the \textbf{\texttt{LogisticsAgent}} to move the animal.
...
\subsubsection{Role and Nature of the Agent}
Acts as a read-only bridge between the SPADE simulation and the NiceGUI dashboard. \textbf{It is not a BDI agent} (it makes no decisions and has no \texttt{.asl} file): it is a purely reactive SPADE agent that projects the grid state to the screen without modifying the environment.
\subsubsection{Whom the Agent Communicates With}
\begin{itemize}
...
\end{enumerate}
\subsubsection{Python Implementation (\texttt{src/tamagotchi\_wild/agents/visualization\_agent.py})}
...
\texttt{accepted=False} response with an accompanying \texttt{refuse} message, 
...
defined in \texttt{environment\_actions.py}, implements an asynchronous 
